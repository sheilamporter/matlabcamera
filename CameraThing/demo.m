% the 'build' script will compile the C++ code i wrote.
% run that script every time you the C++ code changes.
...
% you'll have to generate the output folder name in matlab,
% because every time you call a C++ function from matlab, 
% it's starting from scratch and can't remember what it did last time.
...
% other variables these functions will need
...
% i may expand this later to give you more control over camera settings,
...
camera_capture(camerahandle, quality, frame, outputfolder); % frame 0
frame = frame + 1;
camera_capture(camerahandle, quality, frame, outputfolder); % frame 1
frame = frame + 1;
camera_capture(camerahandle, quality, frame, outputfolder); % frame 2
